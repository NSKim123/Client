...
/*
    컴파일 compile : source code file -> object file로 만드는 것
    ...
    *.exe : 실행파일. 자체적으로 (디스크에서) 메모리로 올려서 실행이 가능한 파일

    *.lib : static library 파일        <-- 쉽게 이야기하면 클래스,함수,변수 등이 들어있는 '바이너리 단위의 재사용' 모듈
    *.dll : dynamic link library 파일  <-- 쉽게 이야기하면 클래스,함수,변수 등이 들어있는 '바이너리 단위의 재사용' 모듈  

    lib <---컴파일 타임에 링킹된다. (정적으로)
    dll <---런타임에 링킹된다. (동적으로)
*/
