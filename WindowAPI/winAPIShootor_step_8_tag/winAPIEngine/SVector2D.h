...
struct SVector2D
{
	...
	//벡터끼리의 덧셈
	//리턴값이 read only      //매개변수는 read only  
	...
	//벡터의 스칼라곱
	const SVector2D operator*(float tScalar) const //<--멤버변수들 read only
	{
	...
	SVector2D Normalize() const
	{
		...
		//<--엡실론 : 무한히 극히 작은 수
		//컴퓨터의 실수 계산은 반드시 오차를 발생시킨다. 그러므로 tLength == 0.0f 라는 표현은 위험하다.
		if(tLength < FLT_EPSILON)   
		{
			//영벡터를 리턴
			return SVector2D::ZERO;   //<--수정필요
		}
		return *this / tLength;
	}
