...
/*
    이번 스텝에서는 color match 게임 알고리즘의 기본을 만들어보자.

    DoFloodFill의 비재귀버전을 가져와 변형하겠다.
...
public:
    ...
    virtual void OnUpdate(float tDeltaTime) override
    {
        ...
        if (CInputMgr::GetInstance()->KeyUp("OnSelectHit"))
        {
            ...
            //int tColorIndex = mBoardAttrib[mCurX][mCurY];
            int tColorIndex = mBoardAttrib[mCurY][mCurX];

            WCHAR szTemp[256] = { 0 };
            wsprintf(szTemp, L"remember CELL : %d\n", tColorIndex);
    ...
    /*
        ...
        이를 위하여 해당 게임 아록리즘에서 검토해야하는 것은 다음의 세가지 정도이다.

        i) 색상을 검사한 곳은 검사했다고 '체크'해둔다.  <-- 이미 체크해둔 곳은 체크하지 않는다.
        ii) 검사한 셀이 다른 색이라면 체크하지 않는다.
        iii) 검사한 셀이 경계를 벗어난 곳이라면 체크하지 않는다.
    ...
    */
    void DoCheckCellAttrib(int tCol, int tRow, int tColorIndex)
    {
        ...
        while (!mIntStack.empty())
        {
            ...
            //다른 색상이라면 체크하지 않는다.
            if (mBoardAttrib[tRow][tCol] != tColorIndex)
            {
                continue;
            }

            //이미 체크해둔 곳이라면 체크하지 않는다.
