...
public:
	...
	//	프리팹 생성과 해제에는 ref개념을 적용하지 않겠다.
	template<typename T>
	T* CreatePrefab(CTexture* tpTex, float tAnchorX, float tAcchorY, SVector2D tPosition)
	{
		T* tpPrefab = new T();;
		...
		//pivot
		tpPrefab->SetAnchorPoint(tAnchorX, tAcchorY);
		tpPrefab->SetTexture(tpTex);

		tpPrefab->SetIsActive(true);   //원본객체 생성시 활성화 상태모 만듦
	...
	//실제객체 생성 함수
	template<typename T>
	T* InstantPrefab(CUnit* tpPrefab)
	{
		T* tpUnit = nullptr;
