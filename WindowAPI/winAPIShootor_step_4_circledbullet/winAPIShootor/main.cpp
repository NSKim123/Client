...
/*
    ...
    <-- 극좌표계 polar coordinate system 의 개념을 알아보자.

    좌표계는 물체의 위치를 지정하는 시스템이다.
    좌표계에는 데카르트 좌표계만 있는 것은 아니다.
    좌표계에는 종류가 많은데
    그 중에서
    '각도'와 '거리'를 구성성분으로 위치를 지정하는 시스템을 극좌표계라고 부른다.
    ...
    <-- 원의 정의에서부터 끌어낸 각도의 개념이다.
    --->그래서 radian은 실수의 연산체계에 통합된다.

    수학 라이브러리나 게임엔진 등의 실제 동작은 radian을 기반으로 한다.
