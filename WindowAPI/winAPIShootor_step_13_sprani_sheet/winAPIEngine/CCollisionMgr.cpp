...
bool CCollisionMgr::DoCollision(CUnit* tThis, CUnit* tOther)
{
	...
	if (tIsCollision)
	{
		...
		//만약, 충돌체 목록에 충돌체가 등록되어 있다면 이전에 어느 순간에 충돌에 일어났던 것이다.)
		bool tIsBe = tpThis->DoCheckBeInCollisions(tpOther);

		//충돌 시작이라면
		if (!tIsBe)
		{
			//충돌 시작이라면 서로 상대의 충돌체를 등록한다.
	...
	else if(tpThis->DoCheckBeInCollisions(tpOther))  //충돌 종료라면
	{
		//충돌 종료라면 서로 상대의 충돌체를 해제한다.
