...
int main()
{
    ...
    //가변배열의 경우 원소를 추가, 삭제하는 연산이 맨 뒤에서 일어나는 것이
    //중간에서 일어나는 것보다 효율적이다.
    //왜냐하면, 배열이란 것이 원소의 주소값을 인덱스에 대응시키는 동작이 일어나는데
    //맨 뒤에 추가, 삭제하는 것은
    //중간에 추가, 삭제하는 것에 비해 해당 연산이 적기 때문이다.
    ...
    for (tItor = tArray_0.begin(); tItor != tArray_0.end(); ++tItor) // 여기서의 ++는 연산자 오버로드로 정의되어 있다.
    {
        cout << *tItor << endl; //iterator는 주소를 나타내므로 간접참조 연산자를 써주어야한다.
    }

    //end() 는 마지막 요소 '너머'의 위치를 가리킨다. 그로인한 for 문의 조건식을 생각해보라.
